...
/* ---------- graph view: obsidian-style graph, stable layout + continuous animation ---------- */
(function(){
  ...
  if(stateEl) stateEl.textContent = 'obsidian graph · louise.ivan / instagram';
  ...
  const nodes = [], links = [];
  const center = {label:'@louise.ivan', r:22, color:'#e8e8ec', x:0, y:0, vx:0, vy:0, kind:'center', born:0, phase:0};
  nodes.push(center);
  const hubs = {};
  ...
  DATA.forEach(d=>{
    ...
    const r = Math.max(2.4, Math.min(13, Math.sqrt(d.views||1)/38));
  ...
  const topSet = new Set(nodes.filter(n=>n.kind==='post').sort((a,b)=>b.d.views-a.d.views).slice(0,20));
  ...
  canvas.addEventListener('mousemove', e=>{
    ...
    if(n && n.kind==='post'){
      ...
      tip.innerHTML = '<b>'+esc(n.label)+'</b><br>'+ d.ts.slice(0,10) + ' · ' + d.niche +
        '<br>'+fmt(d.views)+' views · '+fmt(d.saves)+' saves · '+fmt(d.shares)+' shares';
    } else if(n){
      tip.style.display = 'block';
      const b = canvas.getBoundingClientRect();
      tip.style.left = (e.clientX-b.left+14)+'px'; tip.style.top = (e.clientY-b.top+10)+'px';
